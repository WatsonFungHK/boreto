...
const ShippingMethodForm = () => {
  ...
  const { t } = useTranslation("common", { keyPrefix: "shippingMethod" });
  ...
  useEffect(() => {
    if (!isNew) {
      const fetchItem = async () => {
        try {
          setIsLoading(true);
          const item = await getItem(`/api/shipping-method/${id}`);
          if (item) {
            reset(item);
          }
        } catch (err) {
          toast.error("Error fetching shipping method");
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const onSubmit = async (data: FormData) => {
    try {
      setIsLoading(true);
      const response = await upsertItem(`/api/shipping-method/${id}`, data);
      router.push("/company/shipping-method");
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="h5">
        {isNew ? t("create-shipping-method") : t("update-shipping-method")}
      </Typography>
